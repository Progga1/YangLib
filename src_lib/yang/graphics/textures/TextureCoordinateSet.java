...
public abstract class TextureCoordinateSet {
	...
	public TextureCoordinatesQuad createTexCoordsAbsBiased(float left,float top,float right,float bottom, float biasX, float biasY) {
		final TextureCoordinatesQuad texCoords = new TextureCoordinatesQuad().initBiased(left,top,right,bottom,mTexWidth,mTexHeight,biasX,biasY);
		mTexCoords.add(texCoords);
		return texCoords;
	}
...
//		}
	...
	public TextureCoordinatesQuad createTexCoordsNormalized(float left, float top, float width,float height) {
		final TextureCoordinatesQuad texCoords = new TextureCoordinatesQuad().initBiased(left,top,left+width,top+height, mDefaultBiasX/mTexWidth,mDefaultBiasY/mTexHeight);
		mTexCoords.add(texCoords);
		return texCoords;
	}
	...
	public TextureCoordinatesQuad[] register(TextureCoordinatesQuad[] texCoords) {
		for(final TextureCoordinatesQuad coords:texCoords)
			register(coords);
		return texCoords;
	}

	public TextureCoordinatesQuad[] createSequence(TextureCoordinatesQuad texCoords,int countX,int countY) {
		final TextureCoordinatesQuad[] result = TextureCoordinatesQuad.createSequence(texCoords, countX,countY);
		register(result);
		return result;
	}
	...
	public void intoRect(TextureCoordBounds bounds) {
		TextureCoordinatesQuad.listIntoRect(mTexCoords, bounds.mValues);
	}

	public void intoRect(float[] values) {
		TextureCoordinatesQuad.listIntoRect(mTexCoords, values);
	}
